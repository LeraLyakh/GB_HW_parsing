...
while page <= 10:
    ...
    for vacancy in vacancies:
       ...
       # print(num)
        ...
        vacancy_id = re.findall("\d+", link[-1])[0]
        #print(vacancy_id)
        #print(len(vacancy_id))
        #print(link)
        link = url + '/vacancy/'+vacancy_id#link[-1].split('?')[0]
...
for vac in vacancies_list:
    ...
    if x == 0:
        vacancies_hh.insert_one(
            {'_id': vac['_id'], 'currency': vac['currency'], 'name': vac['name'], 's_max': vac['s_max'],
             's_min': vac['s_min'], 'link': vac['link'], 'feature': 'new vacancy'})
        print(f"Вакансия {vac['link']} добавлена")
    else:
        print(f"Данная {vac['link']} вакансия уже есть")
